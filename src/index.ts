...
async function main() {
  ...
  const ffmpegCmd =
    ...
    -f x11grab -thread_queue_size 2048 -i $DISPLAY
    -c:v libx264 -preset veryfast -tune zerolatency -b:v ${rate}k -minrate ${rate}k -maxrate ${rate}k -bufsize 6000k -g 30
    -ss 00:00:03 ${ffmpegDurationParams} -pix_fmt yuv420p ${exporter.getFFmpegOutputParams()}  2>> ffmpeg_error.log`.replaceAll(
      /[\n\r\s]+/gm,
      ' ',
    )
